...
const nounBankPath = process.env.NOUN_BANK || 'vocabulary/banks/de/nounbank.json';
...
const { _metadata, ...nounEntries } = nounBank;
...
const valid = validate(nounEntries);
if (valid) {
  const count = Object.keys(nounEntries).length;
